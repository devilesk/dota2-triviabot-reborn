// http://blog.paral.in/interacting-with-steam-and-the-dota-gc/
...
ProtoBuf.loadProtoFile(path.join(__dirname + "/generated/dota_gcmessages.proto"), builder);
...
var Dota2Client = function Dota2Client(steamClient, debug, debugMore) {
  ...
  this._sendClientHello = function() {
    ...
    if (!self._client) {
      ...
      console.log(new schema.CMsgClientHello({
            "engine": Dota2.ESourceEngine.k_ESE_Source2
        }));
      self._client.toGC(self._appid, (Dota2.EGCBaseClientMsg.k_EMsgGCClientHello | protoMask),
        //gcsdk_gcmessages.CMsgClientHello.serialize({})
        new schema.CMsgClientHello({
            "engine": Dota2.ESourceEngine.k_ESE_Source2
        }).encode().toBuffer()
      );
    }
...
handlers[Dota2.EGCBaseClientMsg.k_EMsgGCClientConnectionStatus] = function gcClientConnectionStatus(message) {
  ...
  var status = schema.CMsgConnectionStatus.decode(message).status;
